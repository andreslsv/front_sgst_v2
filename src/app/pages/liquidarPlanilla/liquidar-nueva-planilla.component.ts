...
@Component({
  selector: 'vex-liquidar-nueva-planilla',
  templateUrl: './liquidar-nueva-planilla.component.html',
  styleUrls: ['./liquidar-nueva-planilla.component.scss']
})
export class LiquidarNuevaPlanillaComponent implements OnInit {

  dataNomina: any[] = [];
  displayedColumns: string[] = ['nombre', 'salario_base', 'aux', 'ss', 'dias', 'o_ingreso', 'n_ingreso', 'h_extras', 'deducciones', 'total_pagar', 'acciones'];
  displayedColumns2: string[] = ['nombre', 'salario_base', 'dias', 'intereses', 'ss', 'salud', 'o_ingreso', 'h_extras', 'deducciones', 'total_pagar', 'acciones',  'total'];
  displayedColumns3: string[] = ['nombre', 'total_pagar', 'salario_base', 'aux', 'ss', 'dias', 'o_ingreso', 'deducciones', 'h_extras', 'acciones'];
  ...
  openDesprendible(nomina: any) {
    ...
    const win = window.open(`http://api.soyasesorias.co/desprendible/${nomina}`, '_blank');
    win.focus();
  }
   goBack(){
   this._location.back();
  ...
  save() {
    const r = confirm('¿ESTAS SEGURO QUE DESEAS LIQUIDAR LA PLANILLA?');
    if (r === true) {
      const id = `id: ${this.id = this.route.snapshot.paramMap.get('id')},` ;
  ...
  saveVer() {
      ...
      const queryParams = ` ${id} status: 3, ${seleccion} ${salario_dias} ${valorTotal} ${t_otrosPagos} ${t_valorHorasEx} ${t_deducciones} ${t_valorSS} ${numEmpleado}${totalARL} ${totalSalud} ${totalPension} ${totalCCF} ${totalSENA} ${totalICBF} ${totalVacaciones} ${totalIntereses} ${CesantiasTotal} `;
      const queryProps = 'id, afiliacion_id, t_valorSS, nombre_periodo ';
     this.apiService.setData(queryProps, queryParams, 'SaveNomina').subscribe(
      // this.apiService.SaveNomina(queryParams, queryProps).subscribe(
        (response: any) => {
          this.sending = false;
          ...
          console.log(this.dataId)
          // this.goBack()
          this.openPresupuesto(this.dataId)
          // this.router.navigate(['/app/administration']);
  ...
  loadDetalleNomina() {
    ...
    const queryProps =
      'id, totalNomina, cesantias, persona_id, intereses, vacaciones, dias_valor, ibc, ingreso_noc, totalPlanilla, personaNomina{id, primer_nombre,  primer_apellido,  salario_base}, empleadoNomina{id, aporte_activacion, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, salario_base}, nomina_id, empresa_id, empleado_id, ss, ibc, salud, pension, arl, ccf, sena, icdf, dias_laborados, otros_ingresos, horas_extras, deducciones,  riesgo ';
    this.apiService.getData(queryProps, queryParams, 'getDetalleNomina').subscribe(
    // this.apiService.getDetalleNomina(queryParams, queryProps).subscribe(
      (response: any) => {
  ...
  loadNomina() {
    ...
    const queryProps =
      'id, detalleNomina{id, dias_valor, persona_id,  totalNomina, cesantias, intereses, vacaciones, indemnizacion, aux_transporte, salario_base, totalPlanilla, ingreso_noc, personaNomina{id, primer_nombre,  primer_apellido,  salario_base}, empleadoNomina{id, aporte_activacion, subsidio_transporte, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, salario_base, numero_documento}, nomina_id, empresa_id, empleado_id, ss, ibc, salud, pension, arl, ccf, sena, icdf, dias_laborados, otros_ingresos, horas_extras, deducciones,  riesgo },  empresa{id, periodo, afiliacion_id, contacto, telContacto, tipoEmpresa, razon_social, direccion, email, departamento, ciudad, numDocumento, dv, telEmpresaMovil, telEmpresaFijo, representanteLegal, contacto, email_responsable, email_contacto, telefono_responsable, arl, riesgo, caja_cf }, empresa_id, periodo, nombre_periodo, numEmpleado, valorTotal, t_valorSS, t_valorHorasEx, t_otrosPagos, t_deducciones, status, numPlanilla, created_at, afiliacion_id, ';
  ...
  loadEmpleadoPaginator() {
    ...
    this.apiService.getData(queryProps, queryParams, 'empleadoPagination').subscribe(
